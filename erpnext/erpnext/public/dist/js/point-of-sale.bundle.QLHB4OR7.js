(() => {
  ...
  var import_onscan = __toModule(require_onscan());
  ...
  erpnext.PointOfSale.ItemCart = class {
    ...
    make_no_items_placeholder() {
      ...
      this.$cart_items_wrapper.html(`<div class="no-item-wrapper">No items in cart</div>`);
    }
    get_discount_icon() {
    ...
    make_cart_numpad() {
      ...
      this.$numpad_section.append(`<div class="numpad-btn checkout-btn" data-button-value="checkout">Checkout</div>`);
    }
    bind_events() {
      const me = this;
    ...
    render_net_total(value) {
      ...
      this.$totals_section.find(".net-total-container").html(`<div>Net Total</div><div>${format_currency(value, currency)}</div>`);
      this.$numpad_section.find(".numpad-net-total").html(`<div>Net Total: <span>${format_currency(value, currency)}</span></div>`);
    }
    render_grand_total(value) {
      const currency = this.events.get_frm().doc.currency;
      this.$totals_section.find(".grand-total-container").html(`<div>Grand Total</div><div>${format_currency(value, currency)}</div>`);
      this.$numpad_section.find(".numpad-grand-total").html(`<div>Grand Total: <span>${format_currency(value, currency)}</span></div>`);
    }
    render_taxes(taxes) {
      if (taxes.length) {
        const currency = this.events.get_frm().doc.currency;
      ...
      map.forEach((m) => {
        if (m.condition) {
          m.visible_btns.forEach((b) => {
            ...
            this.$summary_btns.append(`<div class="summary-btn btn btn-default ${class_name}-btn">${b}</div>`);
          });
        }
      });
...
`);
    ...
    set_pos_profile_data() {
      ...
      if (this.pos_profile && !this.frm.doc.pos_profile)
...
//# sourceMappingURL=point-of-sale.bundle.QLHB4OR7.js.map
